"use client";

import { useRouter } from "next/navigation";
import { useState } from "react";
import { Copy, LoaderCircle, Send, UserPlus } from "lucide-react";
import type { MerchantInvitation, MerchantSalon } from "@/lib/domain/merchant";

const roleLabels = {
  admin: "Админ",
  manager: "Менеджер",
  staff: "Сотрудник",
};

export function MerchantInvitationPanel({
  invitations,
  salons,
}: {
  invitations: MerchantInvitation[];
  salons: MerchantSalon[];
}) {
  const router = useRouter();
  const manageableSalons = salons.filter(
    (salon) => salon.role === "owner" || salon.role === "admin",
  );
  const [status, setStatus] = useState<"idle" | "saving" | "error">("idle");
  const [errorMessage, setErrorMessage] = useState("");
  const [copiedId, setCopiedId] = useState<string | null>(null);

  async function createInvitation(event: React.FormEvent<HTMLFormElement>) {
    event.preventDefault();
    const formData = new FormData(event.currentTarget);
    setStatus("saving");
    setErrorMessage("");

    const response = await fetch("/api/merchant/invitations", {
      body: JSON.stringify({
        inviteeName: formData.get("inviteeName"),
        phone: formData.get("phone"),
        role: formData.get("role"),
        salonId: formData.get("salonId"),
        telegramUsername: formData.get("telegramUsername"),
      }),
      headers: { "Content-Type": "application/json" },
      method: "POST",
    });

    if (response.ok) {
      event.currentTarget.reset();
      setStatus("idle");
      router.refresh();
      return;
    }

    const payload = await response.json().catch(() => null);
    setErrorMessage(payload?.error ?? "Не удалось создать приглашение");
    setStatus("error");
  }

  async function copyInvite(invitation: MerchantInvitation) {
    await navigator.clipboard.writeText(invitation.inviteLink);
    setCopiedId(invitation.id);
    window.setTimeout(() => setCopiedId(null), 1800);
  }

  return (
    <section className="rounded-[24px] border border-[var(--rose-line)] bg-white p-5 shadow-[var(--shadow-subtle)] sm:p-6">
      <div className="flex flex-col gap-3 border-b border-[var(--line)] pb-5 md:flex-row md:items-end md:justify-between">
        <div>
          <p className="flex items-center gap-2 text-xs font-extrabold uppercase tracking-[0.14em] text-[var(--rose-deep)]">
            <UserPlus aria-hidden className="h-4 w-4" />
            Доступы
          </p>
          <h2 className="mt-2 text-3xl font-extrabold tracking-[-0.02em]">
            Приглашения команды
          </h2>
        </div>
        <span className="w-fit rounded-full bg-[var(--brand-fog)] px-3 py-1 text-sm font-extrabold text-[var(--brand-plum)]">
          {invitations.length} invite
        </span>
      </div>

      {manageableSalons.length > 0 ? (
        <form onSubmit={createInvitation} className="mt-5 grid gap-3 lg:grid-cols-[1fr_1fr_1fr_1fr_auto]">
          <select className="merchant-input" name="salonId" required>
            {manageableSalons.map((salon) => (
              <option key={salon.id} value={salon.id}>
                {salon.name}
              </option>
            ))}
          </select>
          <select className="merchant-input" name="role" required>
            <option value="staff">Сотрудник</option>
            <option value="manager">Менеджер</option>
            <option value="admin">Админ</option>
          </select>
          <input
            className="merchant-input"
            name="inviteeName"
            placeholder="Имя"
          />
          <input
            className="merchant-input"
            name="phone"
            placeholder="+996..."
            type="tel"
          />
          <input
            className="merchant-input lg:col-span-2"
            name="telegramUsername"
            placeholder="@telegram_username"
          />
          <button
            className="inline-flex min-h-12 items-center justify-center gap-2 rounded-full bg-[var(--brand-plum)] px-5 text-sm font-black text-white shadow-[var(--shadow-cta)] disabled:opacity-70 lg:col-span-3 lg:w-fit"
            disabled={status === "saving"}
            type="submit"
          >
            {status === "saving" ? (
              <LoaderCircle aria-hidden className="h-4 w-4 animate-spin" />
            ) : (
              <Send aria-hidden className="h-4 w-4" />
            )}
            Создать ссылку
          </button>
        </form>
      ) : null}

      {errorMessage ? (
        <p className="mt-4 rounded-[14px] border border-red-200 bg-red-50 px-4 py-3 text-sm font-bold text-red-700">
          {errorMessage}
        </p>
      ) : null}

      <div className="mt-5 grid gap-3">
        {invitations.length === 0 ? (
          <p className="rounded-[18px] border border-dashed border-[var(--rose-line)] bg-[var(--porcelain)] px-4 py-6 text-sm font-bold text-[var(--muted)]">
            Создайте приглашение, отправьте ссылку сотруднику в Telegram, и после
            принятия он появится в команде салона.
          </p>
        ) : null}
        {invitations.map((invitation) => (
          <article
            className="grid gap-3 rounded-[18px] border border-[var(--rose-line)] bg-[var(--porcelain)] p-4 md:grid-cols-[1fr_auto]"
            key={invitation.id}
          >
            <div>
              <p className="text-base font-black text-[var(--ink)]">
                {invitation.inviteeName ?? invitation.telegramUsername ?? "Сотрудник"}
              </p>
              <p className="mt-1 text-sm font-bold text-[var(--muted)]">
                {roleLabels[invitation.role]} ·{" "}
                {invitation.acceptedAt ? "принято" : "ожидает принятия"}
              </p>
              <p className="mt-2 break-all text-xs font-bold text-[var(--soft)]">
                {invitation.inviteLink}
              </p>
            </div>
            <button
              className="inline-flex min-h-11 items-center justify-center gap-2 rounded-full border border-[var(--rose-line)] bg-white px-4 text-xs font-black text-[var(--brand-plum)]"
              onClick={() => void copyInvite(invitation)}
              type="button"
            >
              <Copy aria-hidden className="h-3.5 w-3.5" />
              {copiedId === invitation.id ? "Скопировано" : "Скопировать"}
            </button>
          </article>
        ))}
      </div>
    </section>
  );
}
